...
## 설계 원칙: 동기화를 임의로 강제하지 않습니다

이 프로그램은 **동기화 클라이언트를 자동으로 실행하거나 강제로 동기화를 시작(kick)하지 않습니다.**
오직 "이미 진행 중인 작업이 끝났는지"만 **관찰**하고, 끝났을 때 PC를 종료합니다.

그 이유는 다음과 같습니다.

- **사용자의 의도를 존중합니다.** OneDrive·Google Drive를 일부러 종료하거나 일시중지해 둔
  사용자가 있을 수 있습니다(예: 특정 작업을 동기화하고 싶지 않을 때). 프로그램이 이를 멋대로
  다시 켜거나 동기화를 강제하면 사용자의 명시적 의도를 거스르게 됩니다.
- **공식적인 "지금 동기화" 명령이 없습니다.** OneDrive·Google Drive 모두 외부에서 동기화를
  즉시 시작시키는 공개 명령(API/CLI)을 제공하지 않습니다. 강제로 흉내 내려면 클라이언트를
  재시작하는 등 부작용이 큰 방법(진행 중 전송 중단, 가상 드라이브 불안정 등)뿐이라, 안전하지 않습니다.

대신, "동기화 주기가 길어 잠깐 멈춘 구간을 완료로 오판"하는 위험은 아래 안전장치로 대비합니다.

- 첫 측정은 무조건 **'활동 중'**으로 처리 — 시작하자마자 종료되는 일 없음
- 시작 후 **최소 가동 시간**(`MIN_RUNTIME_SECONDS`, 기본 30초) 동안 완료 판정 보류
- **연속 안정화 시간**(`STABILIZE_SECONDS`, 기본 90초) 동안 끊김 없이 잠잠해야 '완료'로 판정

동기화 주기가 길어 더 보수적으로 기다리고 싶다면, 클라이언트를 강제로 깨우는 대신
[config.py](config.py)의 `STABILIZE_SECONDS` 값을 늘리세요(예: 90 → 180초).
